...
$${\sigma }_{z}=f\left({\theta }_{z}\right)=\frac{1}{2}{\sigma }_{\theta }\pm \frac{1}{2}\sqrt{4Y_{p}^{2}-3{\sigma }_{\theta }^{2}}$$
...
$$f\prime \left({\sigma }_{\theta }\right)=\frac{1}{2}+\frac{3}{2}\frac{-{\sigma }_{\theta }}{\sqrt{4Y_{p}^{2}-3{\sigma }_{\theta }^{2}}}=0$$
or
$${\sigma }_{\theta }=\frac{1}{\sqrt{3}}Y_{p}=0.5774{Y}_{p}$$

Substitute back into $f \left({\sigma }_{\theta}\right)$

$${\sigma }_{z}=\frac{1}{2}\frac{1}{\sqrt{3}}Y_{p}+\frac{1}{2}\sqrt{3}Y_{p}$$

or

$${\sigma }_{z}=1.1547{Y}_{p}$$

Note when radical is 0:	

$${\sigma }_{\theta }=\frac{1}{2}{\sigma }_{z}$$

These two equations yield the point of maximum tensile stress 

Special note! $1.1547 = \sqrt{4/3}$ & $0.5774 = \frac{1.1547}{2}$

2D envelope is symmetrical, reciprocate for maximum hoop stress.

$${\sigma }_{\theta }=1.1547{Y}_{p}$$

&

$${\sigma }_{z}=0.5774{Y}_{p}$$

For case of σθ = σz insert into original equation	 

$${\sigma }_{z}=\frac{1}{2}{\sigma }_{z}\pm \frac{1}{2}\sqrt{4Y_{p}^{2}-3{\sigma }_{z}^{2}}$$

or

$${\sigma }_{z}=Y_{p}={\sigma }_{\theta }$$

Create curve with following points, symmetrical about x=y
- Maximum tensile stress	
    - ${\sigma }_{z}=1.1547{Y}_{p}$
    - ${\sigma }_{\theta }=0.5774{Y}_{p}$
- Maximum hoop stress
    - ${\sigma }_{\theta }=1.1547{Y}_{p}$
    - ${\sigma }_{z}=0.5774{Y}_{p}$
- σθ = σz = Yp
...
$${\sigma }_{r}=\frac{\left(1-\frac{R_{o}^{2}}{r^{2}}\right)}{\left(\frac{R_{o}^{2}}{r^{2}}+1\right)}{\sigma }_{\theta }$$

or

$${\sigma }_{r}=C{\sigma }_{\theta }$$
...
$$4Y_{p}^{2}=3{\left(C^{2}-2C+1\right)\sigma }_{\theta }^{2}$$

or for maximum hoop

$${\sigma }_{\theta }=\frac{1}{\sqrt{3\left(C^{2}-2C+1\right)}}2Y_{p}$$
...
$$4{\left(C^{2}-C+1\right)Y}_{p}^{2}=3{\left(C^{2}-2C+1\right)\sigma }_{z}^{2}$$

or max tensile

$${\sigma }_{z}=\sqrt{\frac{\left(C^{2}-C+1\right)}{3\left(C^{2}-2C+1\right)}}2Y_{p}$$
...
For case of σθ = σz 	
$${\sigma }_{z}=\frac{1}{2}{\left(C+1\right)\sigma }_{z}\pm \frac{1}{2}\sqrt{4Y_{p}^{2}-3{\left(C^{2}-2C+1\right)\sigma }_{z}^{2}}$$
or	
$${\sigma }_{z}=\sqrt{\frac{1}{\left(C^{2}-2C+1\right)}}Y_{p}$$

For case of σz or σθ = 0
$$Y_{p}=\sqrt{{\left(C^{2}-C+1\right)\sigma }_{\theta }^{2}}$$	or	$${\sigma }_{z}={\sigma }_{\theta }=\sqrt{\frac{1}{\left(C^{2}-C+1\right)}}Y_{p}$$
...
- Max tension and associated hoop		$${\sigma }_{z-max}=\sqrt{\frac{\left(C^{2}-C+1\right)}{3\left(C^{2}-2C+1\right)}}2Y_{p}$$

$${\sigma }_{\theta }=\frac{\left(C+1\right)}{\left(C^{2}-C+1\right)}\sqrt{\frac{\left(C^{2}-C+1\right)}{3\left(C^{2}-2C+1\right)}}Y_{p}$$

- σθ = σz		
    - ${\sigma }_{z}=\sqrt{\frac{1}{\left(C^{2}-2C+1\right)}}Y_{p}$
- σz = 0		
    - ${\sigma }_{z}=\sqrt{\frac{1}{\left(C^{2}-C+1\right)}}Y_{p}$
- Torque (not plotted for ellipse)	
    - $\tau =\frac{2\bullet Torque\bullet r}{\pi (R_{o}^{4}-R_{i}^{4})}$	use r = Ro 
- Uniaxial Burst (Barlow’s)		
    - ${Burst}_{uniaxial}=Y_{p}\ast \frac{(OD-ID)}{OD}$
- Temperature Derating		
    - $Y_{p,\ \ adj\ Temp}=Y_{p}\left(1-0.0003\left(Temperature-75\right)\right)$
...
- Solve for σz	
    - ${\sigma }_{z}=\frac{1}{2}{\left(C+1\right)\sigma }_{\theta }\pm \frac{1}{2}\sqrt{4Y_{p}^{2}-3{\left(C^{2}-2C+1\right)\sigma }_{\theta }^{2}}$	Solve for both sides of ellipse
- Convert to tension and pressure
- Tension = ${\sigma }_{z}Area/{DF}_{tension}$
...
Transitional Points	
- Yield to Plastic
    - ${\left(\frac{D}{t}\right)}_{Y\ to\ P}=\frac{\sqrt{{\left(A-2\right)}^{2}+8\left(B+\frac{C}{Y_{p}}\right)}+\left(A-2\right)}{2\left(B+\frac{C}{Y_{p}}\right)}$
- Plastic to Transition
    - ${\left(\frac{D}{t}\right)}_{P\ to\ T}=\frac{Y_{p}\left(A-F\right)}{C+Y_{p}\left(B-G\right)}$
- Transition to Elastic
    - ${\left(\frac{D}{t}\right)}_{T\ to\ E}=\frac{2+\frac{B}{A}}{\frac{3B}{A}}$

Solutions
- Yield, analytic solution
    - $P_{Y}=2Y_{p}\frac{\frac{D}{t}-1}{{\left(\frac{D}{t}\right)}^{2}}$
- Plastic, empirical solution
    - $P_{P}=Y_{p}\left(\frac{A}{\frac{D}{t}}-B\right)-C$
- Transition, empirical solution
    - $P_{T}=Y_{p}\left(\frac{F}{\frac{D}{t}}-G\right)$
- Elastic, analytic solution
    - $P_{E}=\frac{2E}{1-v^{2}}x\frac{1}{\left(\frac{D}{t}\right){\left(\frac{D}{t}-1\right)}^{2}}$
